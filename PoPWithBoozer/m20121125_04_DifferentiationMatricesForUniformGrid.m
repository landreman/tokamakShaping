...
%      not xMin.  This option is like scheme=1 but more accurate.
% 80 = Periodic with a grid point at xMin but not xMax.  Upwinding to the
...
% Create grid:
switch scheme
    case {0, 1, 10, 11, 20, 21, 30, 31, 40, 41, 50, 51, 60, 61, 70, 71, 80, 90}
        % Periodic
...
switch scheme
    ...
    case {80,81,82}
        % Stencil with 1 point on 1 side and 2 points on the other side,
        % upwinding to the left.
        % For DD, it turns out that the most accurate stencil using this
        % points is just the familiar 1,-2,1 stencil.
        if N<5
            error('N must be at least 5.')
        end
        D = ( (1/3)*diag(ones(N-1,1),1) + 0.5*diag(ones(N,1), 0) - 1*diag(ones(N-1,1),-1) + (1/6)*diag(ones(N-2,1),-2))/dx;
        DD = (-2*diag(ones(N,1),0) + diag(ones(N-1,1),1)+diag(ones(N-1,1),-1))/dx2;
...
switch scheme
    ...
    case {80,81}
        % Upwinded, 1 point on 1 side, 2 points on the other
        %D = ( (1/3)*diag(ones(N-1,1),1) + 0.5*diag(ones(N,1), 0) - 1*diag(ones(N-1,1),-1) + (1/6)*diag(ones(N-2,1),-2))/dx;
        D(end,1) = 1/(3*dx);
        D(1,end-1) = 1/(6*dx);
        D(2,end) = 1/(6*dx);
