...
function [out]=m20120922_04_read_eqdsk(filename)
...
%end
if fid<0
    error(['Unable to open desired file ',filename])
end
frewind(fid);
...
if out.switch~=3,
	%disp(['Warning, i3=' num2str(out.string) ', unknown file structure...'])
...
% MJL 20151209 Sometimes there is a string, sometimes not.
out.rboxlength=fscanf(fid,'%e',1);	% bow width 
if numel(out.rboxlength)==0
    dummy=fscanf(fid,'%c',8);
    out.rboxlength=fscanf(fid,'%e',1);	% bow width
end
